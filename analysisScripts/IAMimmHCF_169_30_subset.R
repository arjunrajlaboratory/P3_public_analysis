...
if(!dir.exists(paste0(graphDir, '/transdiff/HCF_169-30/'))) {
  dir.create(paste0(graphDir, '/transdiff/HCF_169-30/'))
}

experiments = c('20190701_IAM11169-30-subset/','20190629_HCF30-subset/')
...
scanSpotCounts$condition <- factor(scanSpotCounts$condition, levels = levels(scanSpotCounts$condition)[c(8,2,3,5,1,4,7,6)])

set.seed(7435)
scanScatters12 <- ggplot(scanSpotCounts %>% filter(wellName %in% c('w1', 'w2')), aes(alexa, cy)) +
  geom_jitter() +
  facet_grid(experiment~condition) +
  theme_bw()
ggsave(scanScatters12, file = paste0(graphDir, '/transdiff/HCF_169-30/HCF_169-30_scan_counts_empty_7F_scatter.pdf'), width = 10, height = 10, useDingbats = F)  
