...
tallTableName = paste0(workingDir, "RNAtagTest6_conditionsPositions_tall.csv")
spreadTableName = paste0(workingDir, "RNAtagTest6_conditionsPositions_wide.csv")

stockPositions = as_tibble(read.table(file = paste0(workingDir, "pickingDrugs/20171106_drugPairsWithPairID.txt"), sep = "\t", header = T)) # TBD

set.seed(8831) # seed 8831 for Test6
...
drugPairs = paste0("pair_", as.character(1:25))
# stockPositions$pairIDforPlating = drugPairs
# stockPositions$Test6StocksPlatePosition = stockPositions$stockWellID
# write.table(stockPositions, file = paste0(workingDir, "RNAtagTest6_drugs_expressionTargetsPositions_withDrugIDs.tsv"), sep = "\t", quote = F, row.names = F)
...
stocks = as.character(stockPositions$stockWellID)

# each in triplicate
drugConds = rep(drugPairs, times = 3)
...
tallTableName = paste0(workingDir, "RNAtagTest6_conditionsPositions_stockPositions_tall.csv")
spreadTableName = paste0(workingDir, "RNAtagTest6_conditionsPositions_stockPositions_wide.csv")
